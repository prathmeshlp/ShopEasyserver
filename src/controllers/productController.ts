...
export const validateDiscountHandler = asyncHandler(
...
export const searchProducts = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!q || typeof q !== "string") {
      throw new ApiError(
        400,
        "Search query (q) is required and must be a string",
        [],
        "INVALID_QUERY"
      );
    }
    ...
    res.json(
      new ApiResponse(200, products, "Products search completed successfully")
    );
  }
);
...
export const getReviews = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(
    new ApiResponse(200, product.reviews, "Reviews retrieved successfully")
  );
});
...
export const addReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!userId || !rating || rating < 1 || rating > 5) {
    throw new ApiError(
      400,
      "User ID and rating (1-5) are required",
      [],
